...
func _ready():
	...
	timer.connect("timeout", Callable(get_node("."), "_on_Timer_timeout"))
	add_child(timer)
...
func _physics_process(_delta):
	...
	fps_show = round(lerpf(fps_old, fps_new, 1 - timer.get_time_left()))
	...
	get_node("Panel/TimeLabel").set("theme_override_colors/font_color", Color(1, ((Game.game_time_max - Game.game_time) / Game.game_time_max), ((Game.game_time_max - Game.game_time) / Game.game_time_max)))
	...
	if Game.game_countdown > 0:
		...
		get_node("Panel/TimeLabel").set("theme_override_colors/font_color", Color((Game.game_countdown / Game.GAME_COUNTDOWN_DEFAULT), 1, (Game.game_countdown / Game.GAME_COUNTDOWN_DEFAULT)))
